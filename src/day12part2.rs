...
pub fn main() {
    ...
    println!("{:?}", start);
    println!("{:?}", end);
    grid.get_mut(end.0 as usize).unwrap()[end.1 as usize] = 'a';
    grid.get_mut(start.0 as usize).unwrap()[start.1 as usize] = 'z';
        ...
        for (pt, cost) in frontier.iter() {
            ...
            for (dir_x, dir_y) in [(0, 1), (0, -1), (1, 0), (-1, 0)] {
                ...
                if new_x >= 0 && new_x < m && new_y >= 0 && new_y < n && !visited.contains(&(new_x, new_y)) {
                    let other_c = *grid.get(new_x as usize).unwrap().get(new_y as usize).unwrap();
                    if c - (other_c as i16) <= 1 {
                        if other_c == 'a' {
                            println!("{cost}");
                            break 'outer;
                        }
