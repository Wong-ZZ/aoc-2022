...
pub fn main() {
    ...
    possible_positions.insert((0, 1, 0));
    let end = (m - 1, n - 2);
    for i in 1..20000 {
        let mut flag = false;
        ...
        possible_positions
            ...
            .for_each(|(x, y, foo)| {
                ...
                for (dir_x, dir_y) in [(0, 1), (0, -1), (-1, 0), (1, 0), (0, 0)].iter() {
                    let new_x = *x + *dir_x;
                    let new_y = *y + *dir_y;
                    if ((new_x >= 1 && new_x <= m-2 && new_y >= 1 && new_y <= n-2) || (new_x, new_y) == (0, 1) || (new_x, new_y) == end) && !set.contains(&(new_x, new_y)) {
                        if (new_x, new_y) == goal {
                            next_pos.insert((new_x, new_y, foo+1));
                            if foo+1 == 3 {
                                println!("{i}");
                                flag = true;
                            }
                        } else {
                            next_pos.insert((new_x, new_y, *foo));
                        }
                    }
                }
            });
